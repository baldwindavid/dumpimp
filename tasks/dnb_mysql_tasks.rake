  ...
  namespace :mysql5 do
    ...
    task :check => :environment do
      unless database_config['adapter'] == 'mysql'
        raise "Must be using MySQL, not #{database_config['adapter']}" 
      end
    end
  end
...
def database_config
  @database_config ||= YAML.load_file(File.join(RAILS_ROOT, 'config', 'database.yml'))[RAILS_ENV || 'development']
end
...
def dump_from_mysql(initial_path, gzip, mysql_version = 'mysql')
  ...
  password_option = 
    if database_config.include?('password')
